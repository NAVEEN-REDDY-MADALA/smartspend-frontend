...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .txrow:hover   { background:#f8f8ff !important; }
...
function injectCSS() {
  if (typeof document==="undefined"||document.getElementById("__tx_v4__")) return;
  const s=document.createElement("style"); s.id="__tx_v4__"; s.textContent=CSS;
...
function DetailDrawer({ txn, onClose }) {
  if (!txn) return null;
  const isCredit  = txn._type === "credit";
  const auto      = isAutoTx(txn);
  const merchant  = txn.merchant||txn.merchant_name||txn.source||txn.description||null;
  const dateStr   = fmtDateTime(txn.created_at||txn.date);
  const accentColor = isCredit ? "var(--green)" : "var(--red)";
  const accentBg    = isCredit ? "var(--green-bg)" : "#faf5ff";

  const rows = [
    {emoji:"🏷️", label:"Type",            value: isCredit ? "💰 Income / Credit" : "💸 Expense / Debit"},
    {emoji:"📂", label:"Category",         value: txn.category||txn.category_guess||"—"},
  ...
  return (
    <>
      <div className="overlay" onClick={onClose}
        style={{position:"fixed",inset:0,background:"rgba(17,24,39,.4)",zIndex:100,backdropFilter:"blur(2px)"}}/>
      <div className="drawer"
        style={{position:"fixed",top:0,right:0,bottom:0,width:380,background:"var(--surface)",zIndex:101,boxShadow:"-8px 0 40px rgba(0,0,0,.15)",display:"flex",flexDirection:"column"}}>

        <div style={{padding:"18px 22px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{fontSize:15,fontWeight:700,color:"var(--ink)"}}>Transaction Details</div>
          <button onClick={onClose}
            style={{width:30,height:30,borderRadius:7,border:"1px solid var(--border)",background:"var(--bg)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <Icon d={ICONS.x} size={13} color="var(--ink3)"/>
          </button>
        </div>

        {/* Amount hero */}
        <div style={{margin:"18px 22px 0",padding:"22px",borderRadius:14,textAlign:"center",background:accentBg,border:`1px solid ${isCredit?"var(--green-border)":"var(--purple-border)"}`}}>
          <div style={{fontSize:38,marginBottom:8}}>{CAT_EMOJI[txn.category]||CAT_EMOJI[txn.category_guess]||(isCredit?"💰":"💳")}</div>
          <div style={{fontSize:32,fontWeight:800,color:accentColor,marginBottom:4}}>
            {isCredit?"+":"−"}₹{fmt(txn.amount)}
          </div>
          <div style={{fontSize:13,color:"var(--ink3)",marginBottom:10}}>
            {txn.category||txn.category_guess}{merchant?" · "+merchant:""}
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 22px"}}>
          {rows.map((row,i)=>(
            <div key={row.label}
              style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"13px 0",borderBottom:i<rows.length-1?"1px solid var(--border)":"none"}}>
        ...
        <div style={{padding:"14px 22px",borderTop:"1px solid var(--border)"}}>
          <button onClick={onClose}
            style={{width:"100%",padding:"11px",borderRadius:9,background:"var(--accent)",border:"none",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
            Close
          </button>
        </div>
      </div>
...
export default function Transactions() {
  ...
  const [all,          setAll]         = useState([]);   // combined expenses + incomes
  ...
  // ── Load both expenses (debits) and incomes (credits) ─────────────────────
  async function load() {
    try {
      ...
      // Fetch incomes (credits)
      // const incRes  = await fetch(`${API}/api/incomes/`, { headers });
      const incRes = await fetch(`${API}/api/income/`, { headers });
      const incomes  = incRes.ok ? await incRes.json() : [];

      // Tag each transaction so we know what type it is
      const tagged = [
  ...
  function applyFilters() {
    ...
    // Search by category or merchant
    if (search) {
  ...
  function clearFilters() {
    setSearch(""); setTypeFilter("all"); setCategory(""); setDateFrom(""); setDateTo(""); setActivePreset(null);
  }
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{background:"var(--surface)",borderBottom:"1px solid var(--border)",padding:"16px 28px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            ...
            <div style={{fontSize:13,color:"var(--ink3)",marginTop:2}}>
              All your expenses and income in one place · Click any row for details
            </div>
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"24px 28px",background:"var(--bg)"}}>
          ...
          <div className="fade" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:20}}>
            {[
              {label:"Total transactions", val:filtered.length,           sub:"matching filters",              col:"var(--ink)",    bg:"var(--surface)"},
              {label:"Total income",       val:"₹"+fmt(totalCredit),      sub:creditCount+" credit entries",   col:"var(--green)",  bg:"var(--green-bg)"},
              {label:"Total expenses",     val:"₹"+fmt(totalDebit),       sub:debitCount+" debit entries",     col:"var(--red)",    bg:"var(--red-bg)"},
              {label:"Net balance",        val:(totalCredit>=totalDebit?"+":"-")+"₹"+fmt(Math.abs(totalCredit-totalDebit)),
                                           sub:totalCredit>=totalDebit?"More in than out 🎉":"More out than in ⚠️",
              ...
              <div key={s.label} style={{background:s.bg,border:"1px solid var(--border)",borderRadius:10,padding:"16px 20px",boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
                ...
                <div style={{fontSize:18,fontWeight:700,color:s.col,marginBottom:2}}>{s.val}</div>
          ...
          <div className="f1" style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:10,padding:"14px 18px",marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
            ...
            {/* Row 1: type tabs + search + category */}
            <div style={{display:"flex",gap:10,marginBottom:10,flexWrap:"wrap",alignItems:"center"}}>

              {/* Type filter — most important filter, shown first */}
                ...
                {[
                  ...
                  <button key={opt.value} onClick={()=>setTypeFilter(opt.value)}
                    style={{
                      ...
                      color:      typeFilter===opt.value ? "#fff"          : "var(--ink3)",
                      transition:"all .15s",
                    }}>
                    {opt.label}
                  </button>
              ...
              {hasFilters&&(
                <button onClick={clearFilters}
                  style={{display:"flex",alignItems:"center",gap:5,padding:"8px 12px",borderRadius:7,background:"var(--red-bg)",border:"1px solid var(--red-border)",color:"var(--red)",fontSize:12,fontWeight:500,cursor:"pointer",fontFamily:"inherit"}}>
          ...
          <div className="f2" style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:10,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
            ...
            {filtered.length===0 ? (
              <div style={{padding:"48px 20px",textAlign:"center"}}>
                ...
                {hasFilters&&(
                  <button onClick={clearFilters}
                    style={{marginTop:12,padding:"7px 16px",borderRadius:7,background:"var(--accent)",border:"none",color:"#fff",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>
                    Clear all filters
                  </button>
                )}
              </div>
              ...
              const merchant  = t.merchant||t.merchant_name||t.source||t.description||null;
              const dateStr   = fmtDateTime(t.created_at||t.date);
              const isSelected= selected?.id===t.id && selected?._type===t._type;

              // Left border color: green for credit, blue for auto-debit, transparent for manual debit
              ...
              return (
                <div key={`${t._type}-${t.id}-${i}`}
                  ...
                  <div style={{width:34,height:34,borderRadius:8,
                    background: isCredit?"var(--green-bg)":auto?"var(--blue-bg)":"#f5f3ff",
                    display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>
                    {CAT_EMOJI[t.category]||CAT_EMOJI[t.category_guess]||(isCredit?"💰":"💳")}
                  </div>

                  {/* Category */}
                  <div style={{fontSize:13,fontWeight:600,color:"var(--ink)"}}>{t.category||t.category_guess||"—"}</div>
                  ...
                  <div style={{fontSize:14,fontWeight:700,color:isCredit?"var(--green)":"var(--red)"}}>
