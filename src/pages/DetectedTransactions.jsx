...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 768px) {
    ...
    .txn-row         { flex-direction: column !important; padding:14px !important; }
...
const DEFAULT_FILTERS = { type:"all", amount:"all", category:"All", date:"all" };
          ...
          <div key={si} style={{ marginBottom:6 }}>
            ...
            {sec.items.map(item => (
              <Link
                ...
                style={{ alignItems:"center", justifyContent:"space-between", padding:"8px 10px", borderRadius:7, color:"var(--sidebar-text)", fontSize:13, marginBottom:1 }}
              >
        ...
        <div className="txn-actions" style={{ display:"flex", gap:8, marginTop:12 }}>
          <button className="action-btn" onClick={()=>onAccept(txn.sms_hash,txn)} disabled={busy}
            style={{ display:"flex", alignItems:"center", gap:5, padding:"9px 14px", borderRadius:9, border:"none",
              ...
              fontSize:12, fontWeight:700, cursor:busy?"not-allowed":"pointer", fontFamily:"inherit", flex:1 }}>
            {accepting
...
function FilterSheet({ filters, onChange, counts, onClose, isMobile }) {
  ...
  const catOpts = ["All","Food","Travel","Shopping","Entertainment","Bills","Groceries","Medicine","Income","Other"];
  ...
  const hasActive = filters.type!=="all"||filters.amount!=="all"||filters.category!=="All"||filters.date!=="all";
  ...
  const content = (
    <div style={{ padding:"0 16px 20px" }}>
      ...
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:11, fontWeight:700, color:"var(--ink4)", textTransform:"uppercase", letterSpacing:".8px", marginBottom:8 }}>Type</div>
      ...
      {/* Amount */}
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:11, fontWeight:700, color:"var(--ink4)", textTransform:"uppercase", letterSpacing:".8px", marginBottom:8 }}>Amount</div>
        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
          {amountOpts.map(o=>(
      ...
      {/* Date */}
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:11, fontWeight:700, color:"var(--ink4)", textTransform:"uppercase", letterSpacing:".8px", marginBottom:8 }}>Date</div>
        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
          {dateOpts.map(o=>(
      ...
      {filters.type!=="credit" && (
        <div>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {catOpts.map(cat=>(
              <button key={cat} className="chip-btn" style={chip(filters.category===cat)} onClick={()=>onChange({...filters,category:cat})}>
                {CAT_EMOJI[cat]||""} {cat}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="pop-in" style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12, padding:"16px", marginBottom:14 }}>
      {content}
    </div>
  );
}
...
export default function DetectedTransactions() {
  ...
  const [filters,    setFilters]    = useState(DEFAULT_FILTERS);  // ✅ starts clean
  ...
  useEffect(() => {
    ...
    return () => {
      clearInterval(iv);
      setFilters(DEFAULT_FILTERS);   // ✅ reset filters on page leave
      setShowFilter(false);          // ✅ close filter panel on page leave
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.category!=="All") {
      r = r.filter(t=>t.transaction_type==="credit"
  ...
  async function handleAccept(smsHash, txn) {
    const token = localStorage.getItem("token");
    ...
    try {
      ...
      const amount = res.amount??txn.amount;
      showToast(txn.transaction_type==="credit" ? `₹${fmt(amount)} added to Income 💰` : `₹${fmt(amount)} added as Expense ✓`, true);
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh" }}>
      ...
      {toast && (
        <div style={{
          position:"fixed",
          bottom: isMobile?`calc(var(--bottom-nav-h) + 12px)`:24,
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div className="mobile-header" style={{ background:"var(--sidebar-bg)", padding:"14px 16px 12px", justifyContent:"space-between", alignItems:"center", position:"sticky", top:0, zIndex:50 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={()=>setShowFilter(f=>!f)}
              style={{ width:36, height:36, borderRadius:10, border:"1px solid rgba(255,255,255,.2)", background:showFilter?"rgba(255,255,255,.2)":"rgba(255,255,255,.1)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", position:"relative" }}>
          ...
          <div className="page-wrap" style={{ padding:"18px 24px", maxWidth:820 }}>
            ...
            {pending.length===0 && !error && (
              <div className="fade-up" style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"52px 20px", textAlign:"center", boxShadow:"0 1px 6px rgba(0,0,0,.04)" }}>
                ...
                <div style={{ fontSize:13, color:"var(--ink3)", lineHeight:1.6 }}>No SMS payments waiting.<br/>New ones appear here automatically.</div>
              </div>
            )}
